...
public class MongooseSchema implements IFieldChangeListener,IFieldType.External,ITextLinesProducer,IMutableMongooseFieldTypeCollection{

	public static String OUTPUT_FILENAME_PREFIX="ovmsd."; // to mark the output files as generated by Office Vitae's Mongoose Schema Designer!!
	...
	void updateOptions(List<Option> options){
		for(Option option:options)try{optionCollection.updateOption(option);}catch(Exception ex){}
	}
	*/
	...
	*/
	...
	private List<String> getRequiredFieldNames(String prefix){
		Vector<String> requiredFieldNames=new Vector<String>();
		for(Field field:fieldCollection)if(!field.isVirtual()&&field.isRequired())requiredFieldNames.add(prefix+field.getName());
		return requiredFieldNames;
	}
	private String join(List<String> stringList,String separator,String prefix){
    ...
    .then(client => {
		...
		if(!requiredFieldNames.isEmpty()){
			...
			addControllerTextLine("\t\t\terror: 'One of the required fields for updating a "+Utils.capitalize(name)+" document is missing.'");
			addControllerTextLine("\t\t});");
			addControllerTextLine("\t}");
		}
		*/
	...
	private String getCollectionName(String postfix){
		if(collection!=null)return collection.toString().toLowerCase()+postfix;return"";
	}
	public String getOutputFileName(){
		return OUTPUT_FILENAME_PREFIX+getCollectionName(".")+name.toLowerCase();
	}
...
*/
	...
	ITextLinesConsumer getModelTextLinesConsumer(){
		if(modelTextFile==null)modelTextFile=new ITextLinesConsumer.TextFile(new java.io.File("./app/models",getOutputFileName()+".model.js"));
	...
	*/
	...
	List<String> getModelSchemaCreationLines(){
		...
		// end the fields definition argument to the Schema constructor
		...
		// MDH@08NOV2018: ok, here we have a little problem in that if the option collection has a default we might add options from our MongooseSchemaCollection!!!
		//                how do we organize this??? If using getTextLines() like we do now, I suppose we should be able to link OptionCollection to a parent OptionCollection
		//                NOTE by plugging in the option collection of the Mongoose schema collection at the last moment, we can force getting the nondefaults read by the Mongoose schema collection (when available)
		Vector<String> optionTexts=new Vector<String>();
		try{
			optionTexts.addAll(optionCollection.setParent(this.collection!=null?this.collection.getOptionCollection():null).getTextLines());
		}catch(Exception ex){
		...
		if(this.getParent()!=null)if(!this.containsFieldWithName("_id")||!this.getFieldCalled("_id").isEnabled())optionTexts.add("_id:false");
		if(!optionTexts.isEmpty()){
			optionsTextRepresentation.append(",{"); // continue with the options argument of the Schema constructor call
			modelSchemaCreationLines.add(optionsTextRepresentation.toString());
			// now ready to append the options one line at a time
			for(String optionText:optionTexts)modelSchemaCreationLines.add(mongooseSchemaDeclarationPrefix+optionText+",");
			optionsTextRepresentation=new StringBuilder(mongooseSchemaDeclarationPrefix.substring(1)+"}"); // finish the options arguments
		}
		...
		if(hasVirtualFields){
			...
			for(Field field:fieldCollection)if(field.isVirtual()){
				virtualFieldText=new StringBuilder(getDescription().toString()+".virtual('"+field.getName()+"')");
			...
			if(nametypeParts.length>1&&nametypeParts[0].length()>1&&nametypeParts[1].length()>0){
				...
				for(String contentPart:contentParts){
					...
					if(equalPos>1){ // not a flag
						...
						}else if(fieldPropertyName.equalsIgnoreCase("match")){
							field.matchLiteral.setDisabled(fieldPropertyFlag);
							field.matchLiteral.setText(fieldPropertyValue);
	...
	*/
		...
		if(lineCount>0){ // at least a single line with the subschema names
			...
			for(int lineIndex=0;lineIndex<lineCount;lineIndex++){
				...
				if(line.charAt(0)!='-'&&line.charAt(0)!='+'){ // a subschema (name) or option line
					...
					if(colonPos>=0){
						if(colonPos>0)optionCollection.parseOptionValue(new String[]{line.substring(0,colonPos),line.substring(colonPos+1)});
	...
	public final boolean save(){
		...
		// I suppose returning isSynced() is probably best!!!
		return isSynced();
	}
	// convenience methods
