...
public class MongooseSchema implements IFieldChangeListener,IFieldType,ITextLinesProducer,IMutableMongooseFieldTypeCollection{
	...
	public class OptionCollection extends Vector<Option> implements ITextLinesContainer{
		...
		public void produceTextLines()throws Exception{
			...
			Utils.setInfo(MongooseSchema.this,"Number of non-default options: "+producedTextLines.size()+".");
		}
		...
		public Vector<String> getTextLines()throws Exception{
			...
			for(Option option:this)if(!option.isDefault()&&!textLines.add(option.toString()))throw new Exception("Failed to return option '"+option.getName()+"'.");
			Utils.setInfo(MongooseSchema.this,"Number of non-default options: "+textLines.size()+".");
			return textLines;
		}
	}
	...
	*/
	...
	private String[] getControllerTextLines() throws Exception{
		...
		addControllerTextLine(" */");
		addControllerTextLine("const "+Utils.capitalize(name)+"=require('../models/"+OUTPUT_FILENAME_PREFIX+name+".model.js);");
		...
		addControllerTextLine("\t// Step 1. TODO check the received input (in req.body) and return a res.status(400).send({error:<error message>}); with an appropriate error message when something is wrong!");
		addControllerTextLine("\t");
    ...
    .then(client => {
		...
		List<String> requiredFieldNames=getRequiredFieldNames("!req.body.");
		if(!requiredFieldNames.isEmpty()){
			addControllerTextLine("\tif("+String.join("||",getRequiredFieldNames("!req.body."))+"){");
			addControllerTextLine("\t\treturn res.status(400).send({");
			addControllerTextLine("\t\t\terror: 'One of the required fields for updating a "+name+" document is missing.'");
			addControllerTextLine("\t\t});");
			addControllerTextLine("\t}");
		}
		// do the update passing in req.body as a whole
		addControllerTextLine("\t"+Utils.capitalize(name)+".findByIdAndUpdate(req.params."+name+"Id,{$set:req.body})");
		addControllerTextLine("\t.then("+name+" => {");
		addControllerTextLine("\t\tif(!"+name+"){");
		addControllerTextLine("\t\t\treturn res.status(400).send({");
		addControllerTextLine("\t\t\t\terror:'"+Utils.capitalize(name)+" with id ' + req.params."+name+"Id + 'not found.'");
		...
		addControllerTextLine("\t"+Utils.capitalize(name)+".findByIdAndRemove(req.params."+name+"Id)");
		addControllerTextLine("\t.then("+name+" => {");
		addControllerTextLine("\t\tif(!"+name+"){");
		addControllerTextLine("\t\t\treturn res.status(400).send({");
		addControllerTextLine("\t\t\t\terror: '"+Utils.capitalize(name)+" with id ' + req.params."+name+"Id + 'not found.'");
...
*/
		...
		addRoutesTextLine("\tconst "+lowerName+"s=require('../controllers/"+OUTPUT_FILENAME_PREFIX+lowerName+".controller.js');");
	...
	ITextLinesConsumer getModelTextLinesConsumer(){
		if(modelTextFile==null)modelTextFile=new ITextLinesConsumer.TextFile(new java.io.File("./app/models",OUTPUT_FILENAME_PREFIX+name+".model.js"));
